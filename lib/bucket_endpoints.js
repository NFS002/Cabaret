...
function get(call, cb) {
  ...
  if (!isAddress(predecessor)) {
    return cb(new Error( `Invalid predecessor state: ${predecessor}` ));
  }
  ...
  if (
    !isBetween(predecessorId, keyId, this.id) &&
    keyId.compare(this.id) !== 0
  ) {
  ...
  if (!_.has(this.hashtable, key)) {
    return cb(new Error( `Invalid key for get request: ${key}` ));
  }
...
function set(call, cb) {
  ...
  if (
    ...
    return cb(new Error(  `Key out of range: ${predecessorId},${keyId},${this.id}` ));
  }
